...
function forms(formSelector, modalTimerId) {
  ...
  function bindPostData(form) {
    // ф-ия для постинга данных
    form.addEventListener('submit', e => {
      // навешиваем обр. соб-ый на форму
      ...
      (0,_services_services__WEBPACK_IMPORTED_MODULE_1__.postData)('http://localhost:3000/requests', json).then(data => {
        // data - данные из промиса с сервера
        console.log(data);
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  function showModalByScroll() {
    if (window.scrollY + document.documentElement.clientHeight >= document.documentElement.scrollHeight - 1) {
      // проверяем долистана ли страница до конца
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  function hideTabContent() {
    tabsContent.forEach(item => {
      // Скроем весь контект на странице
      item.classList.add('hide');
      item.classList.remove('show', 'fade');
    });
    tabs.forEach(item => {
      // Удаляем у каждого элемента класс активности
...
document.addEventListener('DOMContentLoaded', function () {
  // Создаем обработчик события DOMContentLoaded для document
  const modalTimerId = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_2__.openModal)('.modal', modalTimerId), 50000); // вызов модального окна через 5с
