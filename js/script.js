document.addEventListener('DOMContentLoaded', function() { // Создаем обработчик события DOMContentLoaded для document
    ...
    function openModal() { // добалвяем мод. окно
        modal.classList.add('show'); 
        modal.classList.remove('hide');
        document.body.style.overflow = 'hidden'; // запрещяем прокрутку страницы
        clearInterval(modalTimerId); // останавливаем таймер мод окна
    }
        
    function closeModal() { // прячем мод. окно
    ...
    document.addEventListener('keydown', (e) => {
        if (e.code === 'Escape' && modal.classList.contains('show')) {  // проверка нажатой клавиши, только при открытом мод. окне
            closeModal(); // закрываем мод. окно
        }
    });
    ...
    function showModalByScroll() { // ф-ия по открытию мод. окна при прокрутке вниз
        if (window.scrollY + document.documentElement.clientHeight >= document. 
            documentElement.scrollHeight - 1) { // проверяем на скролл в вниз страницы
            openModal(); // открытия мод. окна

            window.removeEventListener('scroll', showModalByScroll); // удаляем обр. событий при выполнении ф-ии
        }
    }
    window.addEventListener('scroll', showModalByScroll); // вызываем глобальный обр. событий 
    ...
    class MenuCard { 
        constructor(src, alt, title, descr, price, parentSelector, ...classes ) {
            ...
            this.changeToUAH();
            this.classes = classes; // добавляем rest оператор
        }

        changeToUAH() { // метод по переводу валют
            this.price = this.price * this.transfer;
        }

        render() { // метод по отображению элементовов динамически (через js)
            ...
            `;
            this.parent.append(element); // всталяем элемент после parent
        }
    }

    new MenuCard( // Добавляем карты с помощью ф-ии конструктора
        "img/tabs/vegy.jpg",
        "vegy",
        "Меню 'Фитнес'",
    ...
    forms.forEach(item => { // перебор каждой формы для работы
        postData(item);
    });

    function postData(form) { // Ф-ия по отправке данных
        form.addEventListener('submit', (e) => {
            ...
            request.setRequestHeader('Content-type', 'application/json; charset=utf-8'); // Настройка header для json

            const formData = new FormData(form); // создаем конструктор FormData

            const object = {};
            formData.forEach(function(value, key){ // перебираем formData
                object[key] = value; // Перебираем все значения (ключи)
            });
            const json = JSON.stringify(object); // превращаем XMLHttpRequest в JSON
